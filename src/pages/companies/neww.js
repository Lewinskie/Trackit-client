    ...
    >
      {({ setFieldValue, isSubmitting }) => (
        <Form className="w-full p-6 space-y-6">
          ...
          {/* Placeholder for sticky title */}
          <div
            className={`w-full ${
              isSticky
                ? "sticky top-0 z-50 left 0"
                : ""
            }`}
            style={{ transition: "top 0.3s ease" }}
          >
            <h1
              className={`text-2xl mb-9 font-bold text-light-text-h1 dark:text-text-h1 text-center py-2`}
            >
              Create New Company
            </h1>
          </div>
          {/* Add placeholder to maintain spacing */}
          {isSticky && <div />}{" "}
          {/* Adjust height to match the title height */}
          {/* Form Content */}
          <CardWrapper>
            {/* Company Name */}
            <div>
          ...
          <div className="flex justify-center">
            <button
              ...
              className={`py-3 px-6 font-bold text-light-text-h1 dark:text-text-h1 rounded-md transition duration-300 ${
                isSubmitting
